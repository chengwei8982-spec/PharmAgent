...
def parse_args():
    ...
    # Model parameters
    parser.add_argument("--model_name", type=str, default='EGFR',
                       help="Name of the model")
    parser.add_argument("--approved_name", type=str, default='EGFR_approved_2')
...
def create_approved_dataset(args):
    """Create the approved-drug dataset."""
    dataset_params = {
        'root_path': args.data_path,
        'dataset': args.approved_name,
        'seed': args.seed,
    ...
    dataset = PharmaQADataset(**dataset_params)
    return dataset
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(loader, desc="Predicting")):
            try:
                ...
                fps, mds, phar_targets, phar_target_mx, atom_phar_target_map, labels, smiles_embed, smiles_mask = [
                    x.to(device) for x in [fps, mds, phar_targets, phar_target_mx, atom_phar_target_map, labels, smiles_embed, smiles_mask]
                ]
...
def predict_approved(args):
    """Predict the approved-drug dataset."""
    ...
    args.output_dir = os.path.join(args.model_path, 'approved_predictions')
    ...
    # Create the approved-drug dataset
    dataset = create_approved_dataset(args)
    print(f"Loaded {len(dataset)} molecules from approved dataset")
    ...
    output_path = os.path.join(args.output_dir, f'approved_predictions.csv')
