...
from tdc import Evaluator as TdcEvaluator

Spearman_evaluator = TdcEvaluator(name='Spearman')
...
class Evaluator:
    ...
    def eval(self, y_true, y_pred, valid_ids=None):
        ...
        elif self.eval_metric == 'spear':
            return self._eval_Spearman(y_true, y_pred)
        elif self.eval_metric == 'aupr':
            from tdc import Evaluator
            evaluator = Evaluator(name='PR-AUC')
            # y_true: [1, 0, ...]; y_pred: [0.75, 0.23, ...]
            return evaluator(y_true, y_pred)
        elif self.eval_metric == 'mae':
            result = self._eval_mae(y_true, y_pred)
            return result
        elif self.eval_metric == 'pear':
            return self._eval_pear(y_true, y_pred)
        elif self.eval_metric == 'spear,pear':
            return self._eval_Spearman(y_true, y_pred), self._eval_pear(y_true, y_pred)
        elif self.eval_metric == 'rmse,r2':
            rmse_results = self._eval_rmse(y_true, y_pred)
            r2_results = self._eval_r2(y_true, y_pred)
            return rmse_results, r2_results
        elif self.eval_metric == 'rocauc,aupr':
            from tdc import Evaluator
            evaluator = Evaluator(name='PR-AUC')

            auc_results = self._eval_rocauc(y_true, y_pred)
            aupr_results = evaluator(y_true, y_pred)
            return auc_results, aupr_results
        elif self.eval_metric == 'rocauc,aupr,acc':
            from tdc import Evaluator
            evaluator = Evaluator(name='PR-AUC')

            auc_results = self._eval_rocauc(y_true, y_pred)
            aupr_results = evaluator(y_true, y_pred)
            acc = self._eval_acc(y_true, y_pred)
            return auc_results, aupr_results, acc
        else:
            raise ValueError('Undefined eval metric %s ' % (self.eval_metric))

    def _eval_pear(self, y_true, y_pred):
        if (self.mean is not None) and (self.std is not None):
            y_pred = y_pred * self.std[0] + self.mean[0]
        pear, _ = pearsonr(y_true.squeeze(), y_pred.squeeze())
        return pear
    ...
    def _eval_Spearman(self, y_true, y_pred):
        ...
        for i in range(y_true.shape[1]):
            ...
            if (self.mean is not None) and (self.std is not None):
                Spearman_list.append(
                    Spearman_evaluator(y_true[is_labeled, i], y_pred[is_labeled, i] * self.std[i] + self.mean[i]))
            else:
                Spearman_list.append(Spearman_evaluator(y_true[is_labeled, i], y_pred[is_labeled, i]))

        return sum(Spearman_list) / len(Spearman_list)
